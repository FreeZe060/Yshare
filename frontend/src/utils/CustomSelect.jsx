...
const CustomSelect = ({ options, label, value, onChange, multiple = false, isNested = false, className = '' }) => {
    ...
    return (
        <div className="space-y-1 w-full space">
            ...
            <div className={`relative border-2 border-gray-300 hover:border-[#F929BB] rounded transition duration-300 ease-in-out ${className}`}>
                <button
                    ref={buttonRef}
                ...
                {isOpen && (
                    <ul
                        ...
                        className="z-10 absolute bg-white shadow-lg shadow-md mt-2 py-1 border-2 hover:border-[#F929BB] rounded-md focus:outline-none w-full max-h-64 overflow-auto sm:text-sm text-base leading-6 sm:leading-5 transition duration-300 ease-in-out"
                        ...
                        {multiple ? options.map((option, index) => (
                            <li
                                ...
                                className={`cursor-default select-none cursor-pointer relative py-2 pl-4 pr-9 ${highlightedIndex === index
                                    ? "text-white bg-gradient-to-b from-[#550ECA] to-[#F929BB]"
                                    : "text-gray-900"
                                    }`}
                            >
                                <span
                            ...
                            : isNested
                                            ...
                                            {option.subOptions && (
                                                ...
                                                <span className="pr-2 text-gray-500">
                                                    {option.subOptions.filter((subOption) => subOption.selected).length > 0 && (
                                                        <span className="bg-gradient-to-b from-[#550ECA] to-[#F929BB] mr-2 px-1.5 rounded-xl text-white text-xs text-center">
                                ...
                                : options.map((option, index) => (
                                    <li
                                        ...
                                        onMouseEnter={() => setHighlightedIndex(index)}
                                        onMouseLeave={() => setHighlightedIndex(-1)}
                                        className={`select-none cursor-pointer relative py-2 pl-4 pr-9 ${highlightedIndex === index
                                            ? "text-white bg-gradient-to-br from-[#ab0eca] to-[#F929BB]"
                                            : "text-gray-900"
                                            }`}
                                    >
                                        <span
