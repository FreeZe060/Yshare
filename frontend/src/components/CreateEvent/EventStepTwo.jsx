...
const SortableImage = ({ image, index, onRemove }) => {
  ...
  const style = {
    ...
    transition: transition || "transform 250ms ease", // 💨 fluid drag
    zIndex: isDragging ? 50 : 0, // 👆 pour éviter l’overlay du bouton pendant drag
  };
    ...
    return (
        <div
            ...
            {...attributes}
            {...listeners}
            className="relative border-2 border-dashed border-gray-300 h-48 rounded-lg flex items-center justify-center hover:border-blue-400 transition overflow-hidden group"
        >
            <img
                src={image.preview}
                alt={`img-${index}`}
