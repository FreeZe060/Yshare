...
exports.register = async (req, res) => {  
  try {
    ...
    if (newUser) {
      const token = generateToken(newUser); // ✅ génère le token AVANT
  ...
  try {
    ...
    if (user && await bcrypt.compare(password, user.password)) {
      ...
      return res.json({
        ...
        email: user.email,
        role: user.role,
        token,  // Optionnel, mais pour client on pourra le lire du cookie
      });
    } else {
...
exports.getProfile = async (req, res) => {
  try {
    ...
    if (req.params.userId && loggedUser.role === 'Administrateur') {
      const userProfile = await userService.findById(req.params.userId);
      if (!userProfile) {
    ...
    const fullUser = await userService.findById(loggedUser.id);
    if (!fullUser) {
      return res.status(404).json({ message: "Utilisateur non trouvé" });
    }
...
exports.updateProfile = async (req, res) => {
  try {
    const loggedUser = req.user;
    let userIdToUpdate = loggedUser.id;
    if (req.params.userId && loggedUser.role === 'Administrateur') {
      userIdToUpdate = req.params.userId;
    } else if (req.params.userId && Number(req.params.userId) !== Number(loggedUser.id)) {
      return res.status(403).json({ message: "Accès interdit" });
    }

    const currentUser = await userService.findById(userIdToUpdate);
    if (!currentUser) {
      return res.status(404).json({ message: "Utilisateur non trouvé" });
    }
    ...
    if (req.file) {
      if (currentUser.profileImage) {
        const oldImageFilename = path.basename(currentUser.profileImage);
    ...
    const updatedData = { name, lastname, email, password, profileImage };
...
exports.deleteUser = async (req, res) => {
  try {
    ...
    if (loggedUser.role !== 'Administrateur' && Number(loggedUser.id) !== Number(userIdToDelete)) {
...
exports.getUserEventsAdmin = async (req, res) => {
  try {
    if (!req.user || req.user.role !== 'Administrateur') {
      return res.status(403).json({ message: "Accès interdit. Seuls les administrateurs peuvent voir cette information." });
    }
...
exports.adminCreateUser = async (req, res) => {
  try {
    if (!req.user || req.user.role !== 'Administrateur') {
      return res.status(403).json({ message: "Accès interdit. Seuls les administrateurs peuvent créer des utilisateurs." });
    }
